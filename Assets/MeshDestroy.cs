...
public class MeshDestroy : MonoBehaviour
{
    ...
    {
        // Ejecutar en este objeto si tiene MeshFilter
        if (GetComponent<MeshFilter>() != null)
        {
            DestroyMesh();
        }

        // Repetir en todos los hijos
        foreach (Transform child in transform)
        {
            MeshDestroy meshDestroy = child.GetComponent<MeshDestroy>();

            // Si el hijo ya tiene un MeshDestroy, lo usamos
            if (meshDestroy != null)
            {
                meshDestroy.DestroyAllMeshes();
            }
            else
            {
                // Si no, lo agregamos temporalmente para poder ejecutar la lógica
                meshDestroy = child.gameObject.AddComponent<MeshDestroy>();
                meshDestroy.explosionVFX = this.explosionVFX;
                meshDestroy.CutCascades = this.CutCascades;
                meshDestroy.ExplodeForce = this.ExplodeForce;
    ...
    public void DestroyMesh()
    {
        ...
        {
            if (parts.Count > MAX_PARTS)
            {
                Debug.LogWarning("Too many parts generated, stopping cascade.");
                break;
            }
        ...
        Debug.Log("Explosion Instantiated: " + explosion.name);
